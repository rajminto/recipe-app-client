...
const Loader = () => {
  let leftFlameRef = useRef(null);
  let rightFlameRef = useRef(null);
  let centerFlameRef = useRef(null);

  const fireAnimation = () => {
    const tl = gsap.timeline({ repeat: -1 });
    const flames = [leftFlameRef, centerFlameRef, rightFlameRef];
    tl.to(
      flames,
      {
        duration: 0.5,
  ...
  useEffect(() => {
    gsap.set(leftFlameRef, { autoAlpha: 0 });
    gsap.set(rightFlameRef, { autoAlpha: 0 });
    gsap.set(centerFlameRef, { autoAlpha: 0 });
    fireAnimation();
  }, []);

  return (
    <div className={styles.loaderContainer}>
      <svg viewBox='0 0 35.235 35.235'>
        <g>
          <path
          ...
          />
          <path
            d='M31.005,18.677v-0.352H4.479H4.417H0v1.762h4.417v10.918c0,2.338,1.894,4.229,4.228,4.229h18.132
              c2.336,0,4.229-1.891,4.229-4.229V20.439h4.229v-1.762H31.005z'
          />
          <path
            ref={leftFlame => {
              leftFlameRef = leftFlame;
            }}
            d='M11.24,4.235c0.123,0.162,0.254,0.318,0.391,0.465l0.341,0.342c0.216,0.217,0.417,0.434,0.577,0.629
          ...
          <path
            ref={centerFlame => {
              centerFlameRef = centerFlame;
            }}
            d='M15.809,4.235C15.932,4.397,16.064,4.553,16.2,4.7l0.34,0.342c0.216,0.217,0.416,0.434,0.58,0.629
          ...
          <path
            ref={rightFlame => {
              rightFlameRef = rightFlame;
            }}
            d='M19.972,4.235c0.122,0.162,0.253,0.318,0.39,0.465l0.341,0.342c0.216,0.217,0.415,0.434,0.577,0.629
